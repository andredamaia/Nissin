...
export default function Produtos({ data }: any) {
    ...
    const faceamento = tools.filter((tool: any) => tool.categories.includes(6))
    const contorno = tools.filter((tool: any) => tool.categories.includes(7))
    const redondo = tools.filter((tool: any) => tool.categories.includes(8))
    const copia = tools.filter((tool: any) => tool.categories.includes(9))
    const chaves = tools.filter((tool: any) => tool.categories.includes(10))
    const geral = tools.filter((tool: any) => tool.categories.includes(11))
...
export const getStaticProps: GetStaticProps = async () => {
    const { data } = await api.get('tools?per_page=100&_embed&categories=5,6,7,8,9,10,11',  {
        params: {
            _limit: 12,
            _order: 'desc'
        }
